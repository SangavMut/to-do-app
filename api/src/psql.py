...
def add_new_task(task: str, completed: bool = False):
    with conn.cursor() as cursor:
        cursor.execute(
            "INSERT INTO todo (task, completed) VALUES (%s, %s)",
            (task, completed)
        )
        conn.commit()
        return {"detail": "Task added successfully"}

def update_task(task_id: int, new_task: str, completed: bool):
    with conn.cursor() as cursor:
        cursor.execute(
            "UPDATE todo SET task = %s, completed = %s WHERE id = %s",
